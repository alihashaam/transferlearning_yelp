...
class DocumentLevelClusters(object):
    ...
    # Added by us, to get the clusters in code, nut via files:
    def get_clusters(self):
        clusters = [];
        for w in self.words:
            clusters.append([self.get_bitstring(w), w, self.word_counts[w]]);
        return clusters;
...
# Added by us to keep the process in the code, without surrogating in/out text files:
def brownCluster(DT_matrix, maxVocSize, minWordsCount=1, batchSize=500, doLowerCase=True, passedAsTokens = True):
    word_counts = make_word_counts(DT_matrix,
                                   max_vocab_size=maxVocSize,
                                   min_word_count=minWordsCount);
                                   
    c = DocumentLevelClusters(DT_matrix,
                              word_counts, batch_size=batchSize);
    return c.get_clusters();                          

...
# the path to the main csv file to be processed
mainpath = 'Reviews_By_Business/'
# Set the name of the business to be processed
...
def apply_lemmatization(row):
    ...
    #row['text'] = list(bigrams(row['text']))
    return row
...
def remove_null(df):
    ...
    if df.isnull().values.any():
        rows_with_nan = len(df[df.isnull().values.sum(axis=1)>=1])
...
def pre_process_data(df):
    ...
    df = remove_null(df)
    df = df.apply(lambda row: apply_lemmatization(row),axis=1)
...
def divide_and_conquer(TextFileReader):
	...
	for chunk in TextFileReader:
		...
		print 'Indexes '+str(df.first_valid_index())+' to '+str(df.last_valid_index())+' Done.'
...
def merge_and_blend():
	...
	main_df.to_csv('Preprocessed_'+Business+'_Reviews_Golden.csv')
	print Business+' Data Preprocessed for Golden Model.'
